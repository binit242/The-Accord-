...
public class ContactController {
    ...
    public String saveContact(@Valid @ModelAttribute ContactForm contactForm, BindingResult result,
        ...
        if (user == null) {
            session.setAttribute("message", Message.builder()
                    .content("Could not find your account. Please log out and log in again.")
                    .type(MessageType.red)
                    .build());
        ...
        if (contactForm.getContactImage() != null && !contactForm.getContactImage().isEmpty()) {
            ...
            contact.setPicture(fileURL != null ? fileURL : "/images/telephone.png");
            if (fileURL != null) {
                contact.setCloudinaryImagePublicId(filename);
            }

        } else {
            contact.setPicture("/images/telephone.png");
        }
...
@ResponseBody
public long getFavoriteContactCount(Authentication authentication) {
    String email = Helper.getEmailOfLoggedInUser(authentication);
    User user = userService.getUserByEmail(email);
    if (user == null) {
        return 0;
    }
    return contactService.countFavoriteContactsByUserId(user.getUserId());
}
